...
def test_exact_sandbox_run_approval_matches(tmp_path: Path) -> None:
...
def test_sandbox_run_approval_blocks_mismatches(tmp_path: Path) -> None:
    repo = _repo(tmp_path / "repo")
    profile = get_sandbox_profile("no-network-default")
