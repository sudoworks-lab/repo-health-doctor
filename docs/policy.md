...
repo-health-doctor list-allows . --status expired
...
`--status` は report 内で表示する allow entry だけを絞ります。`--fail-on expired` は期限切れ allow が 1 件以上あれば exit code 1、`--fail-on expiring-soon` は `expiring-soon` または `expired` が 1 件以上あれば exit code 1 です。
...
stale allow review を CI に組み込みたい場合は、scan gate と別に `list-allows` を実行します。

```bash
repo-health-doctor validate-policy .
repo-health-doctor list-allows . --fail-on expiring-soon --format json --output /tmp/repo-health-doctor-allows.json
python3 -m json.tool /tmp/repo-health-doctor-allows.json >/dev/null
```

この gate は raw path pattern や reason を出さずに、期限レビューだけを独立して失敗扱いにできます。
