...
現在はHuman approvalを記録済みで、承認対象はSHA-256
`92e6b1e40f330e36af92a3e0ac06a8406f0dba367d15032fbf5c7c7fcc9a5543`の
`rhd-locked-down-v1`である。F036で非defaultの明示選択肢として製品pathへ接続し、
`runtime-default`はdefaultのまま維持した。
Moby default相当の
...
F029で作成したcandidateは当時製品pathへ接続せず、defaultも変更しなかった。
Human判断は`approved`として記録済みで、F036で明示選択肢へ接続した。
...
static contractである。Human final decisionはHosted candidate regression 8/8の確認後に
`approved`として記録し、F036で非defaultの製品pathへ接続した。
...
安全性、完全な隔離を証明しない。Hosted candidate regressionはrun `29764489485`で8/8 passし、
Human final decisionは`approved`として記録済みで、F036で製品pathへ接続した。
...
`candidate_artifact.runtime_regression_state: completed`で、現在のapprovalは`human_approved`、
product connectionは`connected`である。candidateはpackage data、schema、CLI、Docker argvの
非default明示選択肢へ接続した。F030のhistorical regression recordにある
`human_unapproved` / `disconnected`は実測時点の事実として維持する。
...
Humanはcandidate real Docker regressionの全case結果と全failure、tested runtime、未確認runtime、
image digest、profile hashを確認し、exact hashのcandidateを承認した。F036で製品path接続を
完了したが、default変更は行わず、明示選択した場合だけ適用する。
