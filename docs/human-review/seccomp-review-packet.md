...
このpacketはF028の分析資料を正本とし、F029でHuman未承認candidate artifactの記録を
追加したものである。Moby default相当の
...
`mq_notify`、`mq_send`、`mq_unlink`である。candidateはHuman未承認かつruntime回帰未実施で、
package data、schema、CLI、Docker argvの選択肢には接続していない。
