...
新baselineのF026と再生成candidateのF030はHuman shellでの再検証待ちである。F027で完了した
のはworkflowのstatic contractである。したがって、削減候補は実測とHuman reviewに通るまで
削除決定ではない。
...
今回のF026/F030成功証拠へ流用しない。新contractに対するreal Docker結果はHuman shellでの
再検証待ちである。
...
`candidate_runtime_results`に履歴として残し、現在の再検証待ち状態は
`candidate_local_regression`へ分離している。candidateはpackage data、schema、CLI、Docker
argvの選択肢には接続していない。
...
この8 syscall contractは今回のHuman判断に基づく正規化である。runtime回帰とcandidate全体の
最終Human判断は引き続き未完了である。
