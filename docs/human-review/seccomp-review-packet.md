...
`rhd-locked-down-v1`である。F035時点では製品pathへ未接続で、defaultも変更していない。
Moby default相当の
...
F029で作成したcandidateはF035時点では製品pathへ接続せず、defaultも変更しない。
Human判断は`approved`として記録済みである。
...
`approved`として記録したが、F036完了までは製品pathへ接続しない。
...
Human final decisionは`approved`として記録済みであるが、F036完了までは製品pathへ接続しない。
...
product connectionは`disconnected`である。candidateはF036完了まではpackage data、schema、CLI、
Docker argvの選択肢には接続しない。F030のhistorical regression recordにある
...
image digest、profile hashを確認し、exact hashのcandidateを承認した。このapprovalだけでは
製品path接続やdefault変更は成立せず、F036の検証済み接続を別途必要とする。
