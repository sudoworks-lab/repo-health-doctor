...
default scan、gateを要求するscan、`gate-check`、`real-scan`、`sandbox-run`は、
live targetをscanとexecutionの間で再利用しない。最初にbounded、no-follow、
non-executingなsnapshotを作り、以後のstatic scan、real scanner、Dockerはその
snapshotだけを読む。default scanのstable v3 output shapeは変更しない。
...
各Git childはexec前にaddress space、data、CPU、file size、open file、process
count、core dumpのhard resource limitを設定し、独立process groupで起動する。
pack window、delta cache、thread数も固定し、timeout、parser refusal、resource
超過ではprocess group全体をkillしてreapする。parent側のbounded pipeだけを
resource境界として扱わない。
...
runtimeはactual `VerifiedSnapshot`のrepository identity、commit、tree、
`snapshot_id`、`manifest_fingerprint`を、gateとauthorizationの相互一致だけでなく
各artifactの値と個別に照合する。
...
同じ64 KiB bounded、`O_NOFOLLOW`、lstat/open/fstat/read/fstat readerを、
discoveryだけでなくexplicit authorization、runtime reload、`--argv-json`にも
適用する。sandbox-runはCLI validation、runtime revalidation、single-use
reservation前後でauthorizationのfile stateとdigestを照合する。これらのcheckは
local writerとのTOCTOU raceを縮小するが、侵害済みの同一user processに対するOS-level
isolationではない。Discovery does not prove safety, bind an execution subject
by itself, or set `execution_authorized`.
