...
意味しない。Humanが承認した`rhd-locked-down-v1`もpackage dataとして同梱し、
`sandbox-run --seccomp`で明示選択できる。defaultは従来どおり`runtime-default`で、
Moby baselineも引き続き選択できる。
...
`rhd-moby-default-v1`と`rhd-locked-down-v1`だけを`importlib.resources`で解決する。
...
`rhd-locked-down-v1`のprovenanceはMoby baseline hash
`83e021f30d3fbbdabcc4db55bb760d5947e135491ef214d241d9eda5b0f8f2e8`から15 syscallを
除いた266 syscall profileであること、candidate source、Human approvalを検証した
`final-security-gates.json`、local 8/8、Hosted run `29764489485`の8/8、および環境制限を
記録する。profile bytesのSHA-256は
...
- `rhd-locked-down-v1`も明示選択時だけpackage bytesを使い捨てrun rootへmaterializeし、
  同じcontrolled path contractで渡す。candidate docs pathはDocker argvへ渡さない。
...
実装済みsandbox profileと3つのseccomp選択の全組合せは
...
専用testはsource checkoutの2 resourceを解決した後、local wheelを作成して一時領域へ
installし、同じprofile name、provenance、license、profile hashを比較する。locked-downは
source candidate、package resource、installed wheelのhash一致も固定する。
...
seccompが有効であること、syscall削減の一般的妥当性、配備環境全体への適合性を示さない。
...
正規化後baselineは281 syscall、F030実測時点のHuman未承認candidateは8件すべてを除外した266 syscallで
...
8/8 pass、failure 0で完了した。このF030 historical recordは`human_unapproved`かつ
`disconnected`だった実測時点の事実を保持する。その後Humanはexact hashを承認し、Hosted
run `29764489485`でもDocker `28.0.4`、Ubuntu `24.04.4 LTS`、`x86_64`、kernel
`6.17.0-1020-azure`、同じimage digestで8/8 passした。F036ではdefaultを変えず、
`rhd-locked-down-v1`を明示選択肢へ接続した。これらの結果は記録されたruntime、image、OS、
architecture、kernel、固定caseに限定され、一般的なruntime互換性、安全性、完全な隔離を
証明しない。unsupported workloadは失敗する可能性がある。
