...
gate decision fingerprint、policy version、expiry、approver、approval時刻を確認する。
Humanだけが承認済みartifactを提供できる。

artifact discoveryはapprovalではない。発見されたartifactも既存validator、exact binding、
expiry、clean worktree、single-use reservationを迂回できない。BLOCK、QUARANTINE、UNKNOWNは
...
- staleまたはsubject mismatchのevidence、dirty worktree、期限切れまたは再利用されたauthorizationは停止理由である。
