...
Git childはexec前に`RLIMIT_AS`、`RLIMIT_DATA`、`RLIMIT_CPU`、`RLIMIT_FSIZE`、
`RLIMIT_NOFILE`、`RLIMIT_NPROC`、`RLIMIT_CORE`をhard limitとして設定する。
processは独立process groupで起動し、timeout、parse refusal、resource超過時は
group全体をkillしてreapする。Gitのpack window、delta cache、thread数も固定し、
object decompressionがparentのpipe limitより先にhost resourceを無制限消費しない
ようfail-closedにする。

explicit authorizationと`--argv-json`はdiscoveryと同じ64 KiB bounded、
`O_NOFOLLOW`、lstat/open/fstat/read/fstat readerで読む。authorizationはCLI、
runtime reload、single-use reservation前後でdigestとfile stateが一致しなければ
Docker前に拒否する。
...
- Docker `Popen`成功後は、output処理が`Exception`または`BaseException`で中断しても、
  label/cidfile discovery、forced removal、残存確認を`finally`で行う。interruptは
  containerとworkspaceのcleanup attempt後に再送出する。
